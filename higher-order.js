...
let evenNumbers = mixedNumbers.filter((num, i, arr) => num % 2 === 0);
...
let myStrongest = monstersInYourPocket.filter(function(num) {
  if (num.CP > 200) {
    return true
  }
})
...
let orderTotals = orders.map( mult => {
  return mult.price * ( mult.tax + 1 )
})
...
// let bobsTotal = purchases.reduce(function(acc, element, curr) {
